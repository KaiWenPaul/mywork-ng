...
app.controller('home', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
    ...
    // 流量统计
     $cookieStore.put("utm_source", $location.search()['utm_source']);
     $cookieStore.put("utm_medium", $location.search()['utm_medium']);
     $cookieStore.put("utm_campaign", $location.search()['utm_campaign']);
     $scope.visit = function(arr){
        $http.get($scope.url + "/visit/log", $.param({
            member_id: $cookieStore.get('member_id'),
            utm_source: $cookieStore.get('utm_source'),
            utm_medium: $cookieStore.get('utm_medium'),
            utm_campaign: $cookieStore.get('utm_campaign'),
            utm_content: arr
        }),
            { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
        ).success(function (data) {
    ...
    // 键盘回车搜索
    $scope.myKeyup = function (e, arr) {
        var keycode = window.event ? e.keyCode : e.which;
        if (keycode == 13) {
            $scope.goshoplist(arr);
        }
    }
   ...
   $scope.capitalTip('loginPasswd');
    ...
    // 波尔快报
    $http.post($scope.url + "/swInterfaces.api?getNewsRecommendGoods", $.param({

    }),
        { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
    ).success(function (data) {
        if (data["status"] == "ok") {
            $scope.news = data['data']['newsBeanList'];
        } else {
    ...
    // 当时其它时点击无效
    $scope.other = function (arr, brr) {
        if (arr > 0) {
            window.location.href = "index.html#/shoplist?gid=" + arr + "&uuid=" + arr + "&sortname=" + brr;
        }
    }
    //sorthover
    ...
    $scope.sortcenterover = function (arr) {
        $(".sorts>.list li").eq(arr).addClass("act");
    }
    $scope.sortcenterleave = function (arr) {
        $(".sorts>.list li").eq(arr).removeClass("act");
    }
    ...
    $scope.goshoplist = function (arr) {
        ...
        if (actsortval == 0 && actsortgid == 1) {//搜索商家
            window.location.href = "index.html#/sssj?merchantname=" + name;
        } else if (actsortval == -1 && actsortgid == -1) {
            window.location.href = "index.html#/pplist?pinpaiName=" + encodeURIComponent(name);
        } else {
            $location.path("/searchshoplist").search({ sortname: name, uuid: actsortval, gid: actsortgid })
        }
    }
    ...
    // 详情页分类接口细化
    $scope.is_shop = $location.search()['shopid'] ? 1 : 0;
    if ($scope.is_shop == '0') {
        $scope.sorts(-1, 3)
    } else {
        $scope.sorts(-1, 1)
    }
    $scope.sortShop = function () {
        if ($scope.is_shop == '1') {
            $scope.sorts(-1, 3)
        }
    }
    ...
    .controller('homes', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $scope.index = function(arr){
            window.open("index.html#/shop?shopid="+arr);
        }
    ...
    // 活动
    .controller('huodong', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
    ...
    .controller('qyg', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $scope.moreClick = function () {
            if ($("#fenlei .s-s-l-btn .gd").hasClass('act')) {
                $("#fenlei .s-s-l-btn .gd").removeClass('act')
            } else {
                $("#fenlei .s-s-l-btn .gd").addClass('act')
            }
            if ($("#fenlei .s-s-l-pp-list").hasClass('act')) {
                $("#fenlei .s-s-l-pp-list").removeClass('act');
            } else {
                $("#fenlei .s-s-l-pp-list").addClass('act')
            }
        }
        //确认
    ...
    // 搜索商品展示
    .controller('searchshoplist', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore, $sce) {
        $scope.navsortshows(1, 0);
        $scope.scrolltop(0);
        // 获取url参数
        $scope.goods_uuid = $location.search()['uuid'] ? $location.search()['uuid'] : '';
        $scope.goods_id = $location.search()['gid'] ? $location.search()['gid'] : '';
        $scope.sortTie = function () {
            $http.post($scope.url + "/goodsInterfaces.api?getOneClassLine", $.param({
                goods_id: $scope.goods_id,
            }),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
            ).success(function (data) {
                if (data["status"] == "ok") {
                    $scope.sortTies = data["data"];
        ...
        if ($location.search()['hotname']) {
            angular.element('#intext').val($location.search()['hotname']);
        }
        //商品列表
        $scope.htmlData = [];
        $scope.skuData = [];
        $scope.shopl = function (arr, brr, crr, drr, err, frr, grr, hrr, irr) {
            $scope.loading = 1;
            $scope.arr = arr;
            $scope.brr = brr;
            $scope.crr = crr;
            ...
            $scope.hrr = hrr;
            $scope.irr = irr;
            $http.post($scope.url + "/goodsInterfaces.api?searchFilterdGoodsDetailList", $.param({
                goods_name: arr,        //商品名
                parent_id: brr,
                brand_id: drr,          //品牌  
                merchants_id: err,
                sort: frr,              //pc_price/price/assessment/sales 按销量排序，按价格排序,
                sort_way: grr,         //desc/asc 按销量降序 升序
                min_pc_price: hrr,      //最小价格
                max_pc_price: irr,      //最大价格
                page: crr,              //页数       
                limit: 40,
            }),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
                ...
                if (data["status"] == "ok") {
                    ...
                    for (var i = 0; i < data["data"].length; i++) {
                        if (data["data"][i]['type'] == 'goods') {
                            $scope.shoplist = data["data"][i]['goodsBeans'];
                        }
                    }
                    for (var i = 0; i < $scope.shoplist.length; i++) {
                        var html = $sce.trustAsHtml($scope.shoplist[i]['goods_name']);
                        $scope.htmlData.push(html);
                        $scope.skuData.push($sce.trustAsHtml($scope.shoplist[i]['goods_sku']))
                    }
                    $scope.shoptotal = parseInt(data['total']);//总数
                    $scope.pageNum = Math.ceil($scope.shoptotal / 40) == 0 ? '1' : Math.ceil($scope.shoptotal / 40);//总页数
        ...
        // 获取分类条件，相当于缓存
        $scope.shoplsort = function (arr, brr, crr, drr, err, frr, grr, hrr, irr) {
            $scope.loading = 1;
            $scope.arr = arr;
            $scope.brr = brr;
            $scope.crr = crr;
            ...
            $http.post($scope.url + "/goodsInterfaces.api?searchFilterdGoodsDetailList", $.param({
                ...
                if (data["status"] == "ok") {
                    ...
                    $scope.brandData = [];
                    $scope.shoppinpais = data['data']
                    for (var i = 0; i < $scope.shoppinpais.length; i++) {
                        if (data["data"][i]['type'] == 'guide') {
                            $scope.guidelist = data["data"][i]['guideBeans'];
                        }
                    }
                    if ($scope.guidelist) {
                        for (var i = 0; i < $scope.guidelist.length; i++) {
                            if ($scope.guidelist[i]['guide_type'] == 'class') {
                                $scope.classData.push($scope.guidelist[i]);
                            }
                            if ($scope.guidelist[i]['guide_type'] == 'brand') {
                                $scope.brandData.push($scope.guidelist[i]);
                            }
                        }
                    }
                    for (var i = 0; i < data["data"].length; i++) {
        ...
        //获取商品分类名
        $scope.getsortname = $location.search()['sortname'] ? $location.search()['sortname'] : '分类';
        $scope.hotname = $location.search()['hotname'];
        $scope.shoplsort($location.search()['sortname'], $scope.goods_uuid, '1', '', '', '', '', '', '', '');
        //单击。
        $scope.ssclicks = function (arr, brr) {
            if ($("#" + arr).find('.dx').hasClass('act')) {
                return false;
            ...
            if (arr == 'brand') {//品牌
                $scope.shopl($scope.arr, $scope.brr, 1, brr, $scope.err, $scope.grr, $scope.hrr, $scope.irr)
            } else if (arr == 'fenlei') {//分类
                if (brr == '') {
                    $scope.shopl($scope.arr, $location.search()['uuid'] ? $location.search()['uuid'] : '', 1, $scope.drr, $scope.err, $scope.frr, $scope.grr, $scope.hrr, $scope.irr);
                } else {
                    $scope.shopl($scope.arr, brr, 1, $scope.drr, $scope.err, $scope.frr, $scope.grr, $scope.hrr, $scope.irr);
                }
            } else {
                $scope.alerttxt('暂无该分类')
            }
        ...
        // 其他更多
        $scope.moreOtherClick = function () {
            if ($(".pinpai .s-s-l-btn .gd").hasClass('act')) {
                $(".pinpai .s-s-l-btn .gd").removeClass('act')
            } else {
                $(".pinpai .s-s-l-btn .gd").addClass('act')
            }
            if ($(".pinpai .s-s-l-pp-list").hasClass('act')) {
                $(".pinpai .s-s-l-pp-list").removeClass('act');
            } else {
                $(".pinpai .s-s-l-pp-list").addClass('act')
            }
        }
        //确认
        $scope.dbclicks = function (arr) {
            ...
            if ($("#" + arr).find('.dx').hasClass('act')) {
                ...
                $("#" + arr).find('.s-s-l-sp-list li,.s-s-l-pp-list li').each(function (i) {
                    if ($("#" + arr).find('.s-s-l-sp-list li,.s-s-l-pp-list li').eq(i).hasClass('act')) {
                        zrr.push($("#" + arr).find('.s-s-l-sp-list li,.s-s-l-pp-list li').eq(i).attr("val"));
                        // $("#"+arr).find('.s-s-l-sp-list li,.s-s-l-pp-list li').eq(i).removeClass('act')
                    }
                });
                idlist = zrr.join(",");
            } else {
            ...
            if (arr == 'brand') {//品牌
                $scope.shopl($scope.arr, $scope.brr, 1, idlist, $scope.err, $scope.grr, $scope.hrr, $scope.irr)
            } else if (arr == 'fenlei') {//分类
                $scope.shopl(idlist, $scope.brr, 1, $scope.drr, $scope.err, $scope.frr, $scope.grr, $scope.hrr, $scope.irr, $scope.jrr, $scope.krr);
            } else if (arr == 'activity') {//活动
                $scope.shopl($scope.arr, $scope.brr, 1, $scope.drr, $scope.err, $scope.frr, idlist, $scope.hrr, $scope.irr, $scope.jrr, $scope.krr)
        ...
        $scope.sortway = function (arr) {
            ...
            if (arr == 'pc_price') {
                ...
                $scope.shopl($scope.arr, $scope.brr, '1', $scope.drr, $scope.err, arr, way, $scope.hrr, $scope.irr);
            } else {
                if ($scope.sortways == arr) {
                    return false;
                } else {
                ...
                $scope.shopl($scope.arr, $scope.brr, 1, $scope.drr, $scope.err, arr, 'desc', $scope.hrr, $scope.irr)
            }
        }
        //最大最小价格
        $scope.funprice = function () {
            ...
            $scope.shopl($scope.arr, $scope.brr, 1, $scope.drr, $scope.err, $scope.frr, $scope.grr, minprice, maxprice)
        }
        //分页
        $scope.pageclick = function (arr) {
            $scope.htmlData = [];
            ...
            if (arr > 0) {
                $scope.shopl($scope.arr, $scope.brr, arr, $scope.drr, $scope.err, $scope.frr, $scope.grr, $scope.hrr, $scope.irr)
                var anh = $('.shop-l-sorts').offset().top;
                $scope.scrolltop(anh);
            }
        }
    ...
    //商品列表 试剂 仪器 耗材
    .controller('shoplist', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore, $sce) {
        $scope.navsortshows(1, 0);
        $scope.scrolltop(0);
        // 获取url参数
        $scope.goods_uuid = $location.search()['uuid'] ? $location.search()['uuid'] : '';
        ...
        if ($location.search()['uuid']) {
            $scope.shoppinpai($scope.goods_uuid);
        }
        ...
        $scope.hotname = $location.search()['hotname'];
        //一进来判断
        if ($location.search()['uuid'] && $location.search()['sortname']) {
            ...
            $scope.shoplistsort($location.search()['gid']);
            // $scope.shoppinpai($location.search()['uuid']);
        } else if ($location.search()['sortname']) {
            $scope.shopl('', $location.search()['sortname'], 1, '', '', '', '', '', '', '', '');
    ...
    .controller('shop', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore, $sce) {
        ...
        $scope.goods_id = $location.search()['shopid'] ? $location.search()['shopid'] : '';
        ...
        $http.post($scope.url + "/goodsInterfaces.api?getOneGoodsDetail", $.param({
            ...
            if (data["status"] == "ok") {
                ...
                $scope.p_nowprice = $scope.shopdetails.goods_pc_price;//手机价
                $scope.origin_price = $scope.shopdetails.goods_origin_price;//原价
            ...
            $(".s-o-3of5").each(function (i) {
                $(".s-o-3of5").eq(i).find('p span').each(function (k) {
                    if ($(".s-o-3of5").eq(i).find('p span').eq(k).hasClass('act')) {
                        ...
                        selgg_orp = Number(selgg_orp) + Number($(".s-o-3of5").eq(i).find('p span').eq(k).attr("originprice"));

                    }
                })
            })
            $scope.nowprice = Number(selgg_p) + Number($scope.shopdetails.goods_pc_price);
            $scope.p_nowprice = Number(selgg_p) + Number($scope.shopdetails.goods_pc_price);
            $scope.origin_price = Number(selgg_orp) + Number($scope.shopdetails.goods_origin_price);
        }
        //商品评价
            ...
            if (drr == 0) {
                $http.post($scope.url + "/collectionInterfaces.api?insertCollection", $.param({
                    ...
                    if (data["status"] == "ok") {
                        ...
                        $scope.alerttxt(data['error']);
                        if ($(".dpxq-details .btn .sc").hasClass('act')) {
                            $(".dpxq-details .btn .sc").removeClass('act')
                        }
                        if ($(".mbx-nav .mbx-nav-r .sc").hasClass('act')) {
                            $(".mbx-nav .mbx-nav-r .sc").removeClass('act')
                        }
                        if ($(".s-o-5of5 .sc").hasClass('act')) {
                            $(".s-o-5of5 .sc").removeClass('act')
                        }
                    }
                })
            } else if (drr == 1) {
        ...
        $scope.shopcart = function (arr, brr, crr) {//数量，商品id，商家id
            ...
            //     return false;
            // }
            if ($scope.shopdetails.is_supply == 0) {
                // $scope.alerttxt('库存不足！');
                return false;
            }
            var len = $('.s-o-3of5').length;
    ...
    //         link: function (scope, element, attributes) {
    //             element.click(function () {
    //                 //console.log(scope.shopdetails.goods_pc_price)shopdetails.goods_pc_price
    //                 element.addClass("act");
    ...
    //品牌馆123
    .controller('pplist', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore, $sce) {
        $scope.navsortshows(1, 0);
        $scope.scrolltop(0);
        //获取分类
        ...
        $scope.pplistHtml = [];
        // 搜索框进来
        $scope.searchpplist = function (brr) {
            var url = (window.location.href).split('=');
            $http.post($scope.url + "/goodsInterfaces.api?searchBrand", $.param({
                brand_name: decodeURIComponent(url[1]),
                limit: 10,
                page: brr
            }),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
                ...
                if (data["status"] == "ok") {
                    $scope.shoppinpais = data["data"];
                    for (var i = 0; i < $scope.shoppinpais.length; i++) {
                        var html = $sce.trustAsHtml($scope.shoppinpais[i]['brand_name']);
                        $scope.pplistHtml.push(html);
                    }
                    $scope.shoptotal = data['total'];//总数
                    $scope.pageNum = Math.ceil($scope.shoptotal / 20) == 0 ? '1' : Math.ceil($scope.shoptotal / 20);//总页数
                    $scope.nowpageNum = brr;//当前页
        ...
        //获取商品品牌 活动 服务 发货地
        $scope.shoppinpai = function (arr, brr) {
            $http.post($scope.url + "/goodsInterfaces.api?getAllBrandByPage", $.param({
                goods_id: arr,
                limit: 20,
                page: brr
            }),
        ...
        if ($location.search()['pinpaiName']) {
            $scope.searchpplist(1);
        } else {
            $scope.shoppinpai(-1, 1)
        }
        //  $scope.searchpplist(1);
        // 分页
        $scope.pageclick = function (arr) {
            ...
            if (arr > 0) {
                if ($location.search()['pinpaiName']) {
                    $scope.searchpplist(arr);
                } else {
                    $scope.shoppinpai(-1, arr)
                }
                // $scope.shoppinpai(-1,arr)
                $scope.scrolltop(0);
            }
    ...
    .controller('pinpai', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        if ($location.search()['goods_uuid']) {
            $scope.pinpaishop($location.search()['goods_uuid'], 1, '', '', '', '', '', '');
        } else {
            $scope.pinpaishop('', 1, '', '', '', '', '', '');
        }
        ...
        $scope.hotshop = function (arr, brr) {
            ...
            $http.post($scope.url + "/rankingInterfaces.api?getSalesRanking", $.param({
                ...
                if (data["status"] == "ok") {
                    ...
                    $scope.shoptotal = data['total'];//总数
                    $scope.pageNum = Math.ceil($scope.shoptotal / 30) == 0 ? '1' : Math.ceil($scope.shoptotal / 30);//总页数
                    $scope.nowpageNum = brr;//当前页
                    $scope.nowpageNum2 = brr;//当前页 跳转用
                    $scope.pagebox($scope.shoptotal, $scope.pageNum, $scope.nowpageNum)
                } else {
