...
app.controller('home', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
    ...
    $scope.grxx_center = function () {
        $http.post($scope.url + "/memberInterfaces.api?getMemberDetail", $.param({
            ...
            if (data["status"] == "ok") {
                ...
                $scope.payTrustMoney = Number(data['data']['trust_payAmount_current']) + Number(data['data']['trust_payAmount_indebt']);
                $scope.cznum = $scope.payTrustMoney;
                $scope.time = $scope.grxx_centers.trust_paytime.split('-');
            } else {
                $scope.membershow = 0;
                $scope.grxx_centers = ''
            }
    ...
    $scope.regbtn = function (arr, brr, crr, drr, err) {//注册
        ...
        var testreg = /^(?=[\d.]{1,15})[1-9]\d{0,14}(\.\d\d)?$/;
        // /^0\d{2,3}-?\d{7,8}$/;
        if (!myreg.test(arr)) {
    ...
    .controller('wddd', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $scope.is_prize = 0;
        $scope.alert_show = 0;
        $scope.alert = function (arr, brr,crr,drr) {
            if (brr == 1) {
                $scope.alertcon = arr;
                $scope.gift_id =crr;
                $scope.member_coupon_id =drr;
                $scope.alert_show = 0;
            } else {
                $scope.alertcon = arr;
                $scope.gift_id =crr;
       ...
       $scope.Hide_prize = function(){
             $scope.is_prize = 0;
       }
        $scope.isHide = function (arr) {
            if (arr == 1) {
                $scope.isShow = 0;
        ...
        if ($location.search()['orderid']) {
            $http.post($scope.url + "/orderInterfaces.api?getAfterOrderRewards", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                ...
                if (data["status"] == "ok") {
                    if(data['data']['type']!=''){
                       if(data['data']['type']=="gift"){
                         $scope.is_prize = 1;//福袋的弹框
                         $scope.prizelist = data['data']['giftBeans'];
                        }else if(data['data']['type']=="rewards"){
                            $scope.isShow = 1;//现金红包的弹框
                        }else{
                            $scope.isShow = 0;
                            $scope.is_prize = 0;
                        }
        ...
        // 用户领取完奖金后 调用该接口来表示用户已领取
        $scope.receiveReward = function (arr,brr,crr,drr) {
            // arr type类型
            // brr id
            // crr member_coupon_id
            var data={};
            ...
            if(arr=='gift'){
                data.gift_id = brr;
                data.member_coupon_id = crr;
            }else{
                data.member_rewards_id = brr;
            }
            $http.post($scope.url + "/orderInterfaces.api?receiveAfterOrderRewards", $.param(data),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
            ).success(function (data) {
                if (data["status"] == "ok") {
                  window.location.href='core.html#/yhq';
                } else if (data["status"] == "pending" && data["error"] == "token failed") {
                    $scope.relogin()
                } else {
                    $scope.alerttxt(data['error'])
        ...
        $scope.confirmPrize =function(arr,brr,crr){
            $scope.alert_show = 0;
            $scope.receiveReward('gift',brr,crr,'');
            $scope.is_prize = 0;
        }
        $scope.tipShow = function (arr,brr,crr,drr) {
            // arr type类型
            // brr id
            // crr member_coupon_id
            if(arr=='gift'){
                $scope.alert(drr,'',brr,crr);
            }else if(arr=='reward'){
                $scope.tipShow = 1;
                $scope.receiveReward(arr,brr,'','');
            }
        }
        ...
        $scope.ordernum();
        $scope.lijifu = function (arr, brr) {
            window.location.href = "gwc.html#/gopay?orderid=" + base64_encode(arr.toString()) + "&paynum=" + base64_encode(brr.toString());
        }
        //订单列表
        ...
        //在线生成合同
        $scope.lineOrder = function (arr) {
            $http.post($scope.url + "/orderInterfaces.api?downloadContact", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                order_id: arr,
            }),
                ...
                if (data["status"] == "ok") {
                    location.href = data['data']['contact_path'];
                } else if (data["status"] == "pending" && data["error"] == "token failed") {
                    $scope.relogin()
                } else {
                    $scope.alerttxt(data['error'])
    ...
    .controller('ddxq', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
       ...
       $scope.ddxq();
        ...
        //确认收货
        $scope.qrshop = function (arr) {
            $http.post($scope.url + "/orderInterfaces.api?receiveParcel", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                order_parcel_id: arr,
            }),
    ...
    //订单详情
    .controller('jfxq', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        $scope.scrolltop(0)
        $scope.h3nav(1, 0);
        //物流详情
        $scope.wlxq = function (arr) {
        ...
        //积分订单详情
        $scope.jfxq = function(){
           $http.post($scope.url + "/integralInterfaces.api?getMemberIntegralOrder", $.param({
            member_id: $cookieStore.get("member_id"),
            member_token: $cookieStore.get("member_token"),
            order_id: $location.search()['orderid'],
            }),
    ...
    .controller('wdjf', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $scope.integralOrderList = function(arr){
            $http.post($scope.url + "/integralInterfaces.api?getIntegralOrderList", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                order_state:'',
                order_no:'',
        ...
        $scope.integralOrderList(1);
        $scope.wdjflist = function (arr) {
            $http.post($scope.url + "/integralInterfaces.api?getIntegralRecordList", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                page: arr,
                limit: 40,
        ...
        $scope.wdjflist(1);
        // 积分确认收货
        $scope.sure = function(arr){
            $http.post($scope.url + "/integralInterfaces.api?receiveOrder", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                order_id:arr
            }),
    ...
    .controller('yhq', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $scope.receivePrize =function(arr){
            $scope.is_prize = 1;
            $http.post($scope.url + "/couponInterfaces.api?getCouponGiftList", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                member_coupon_id: arr
            }),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
            ).success(function (data) {
                if (data["status"] == "ok") {
        ...
        $scope.tipShow = function(arr,brr,crr,drr){
            $scope.alert(arr,'',brr,crr);
        }
        $scope.confirmPrize = function(arr,brr){
            var data={};
            data.member_id = $cookieStore.get("member_id");
            data.member_token =$cookieStore.get("member_token"); 
            data.type = 'gift';
            data.gift_id = arr;
            data.member_coupon_id = brr;
            $http.post($scope.url + "/orderInterfaces.api?receiveAfterOrderRewards", $.param(data),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
            ).success(function (data) {
                if (data["status"] == "ok") {
                     location.href = "core.html#yhq"
                } else if (data["status"] == "pending" && data["error"] == "token failed") {
                    $scope.relogin()
                } else {
                    $scope.alerttxt(data['error'])
        ...
        // 用户领取完奖金后 调用该接口来表示用户已领取
        $scope.receiveReward = function (arr, brr) {
            $scope.benefits_money = brr;
            $scope.tipShow = 1;
            $http.post($scope.url + "/orderInterfaces.api?receiveRewards", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                member_rewards_id: arr
            }),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
            ).success(function (data) {
                if (data["status"] == "ok") {
       ...
       $scope.back = function(){
        ...
        $scope.getCount = function () {
            $http.post($scope.url + "/swInterfaces.api?getBenefitsCount", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
            }),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
        ...
        $scope.yhqlist = function (arr, brr) {// not_used:未使用 already_used:已使用 expired:过期
            ...
            // })
            // 优惠券接口更新
            $http.post($scope.url + "/swInterfaces.api?getBenefits", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                benefits_state: arr,
                page: brr,
    ...
    .controller('xyed', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $http.post($scope.url + "/othersInterfaces.api?getHtmlDesc", $.param({
         ...
         $scope.grxx_center();
        $scope.xyedjl = function (arr, brr) {
            $scope.arr = arr;//页数初始化
            $scope.brr = brr;//还款状态状态初始化
            $http.post($scope.url + "/memberInterfaces.api?getMemberTrustRecord", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                is_total_payoff: brr,//1 表示 都偿还了，0 表示没偿还 
                page: arr,
                limit: 5,
            }),
                { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }
                ...
                if (data["status"] == "ok") {
                    ...
                    $scope.shoptotal = data['total'];//总数
                    $scope.pageNum = Math.ceil($scope.shoptotal / 5) == 0 ? '1' : Math.ceil($scope.shoptotal / 5);//总页数
                    $scope.nowpageNum = arr;//当前页
                    $scope.nowpageNum2 = arr;//当前页 跳转用
                    $scope.pagebox($scope.shoptotal, $scope.pageNum, $scope.nowpageNum)
                } else if (data["status"] == "pending" && data["error"] == "token failed") {
        ...
        if ($location.search()['payoff']) {
            $('.dd-nav>a').removeClass('act');
            if ($location.search()['payoff'] == '1') {
                $('.dd-nav>a').eq(1).addClass('act');
                $scope.xyedjl(1, '1');
            } else {
                $('.dd-nav>a').eq(0).addClass('act');
    ...
    //   额度还款
    .controller('edhk', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        $scope.scrolltop(0)
        $scope.h3nav(1, 0);
        ...
        // 额度还款alerttxt
        $scope.yhcz = function (arr) {
            $http.post($scope.url + "/orderInterfaces.api?chainPayInsertRechargeOrder", $.param({
                member_id: $cookieStore.get("member_id"),
                member_token: $cookieStore.get("member_token"),
                    ...
                    $http.post($scope.url + "/orderInterfaces.api?insertRechargeOrder", $.param({
                        ...
                        if (data["status"] == "ok") {
                            if (brr == "alipay_pc_direct") {
                                ...
                                location.href = "gwc.html#/wxpay?paynum=" + base64_encode(arr.toString()) + "&orderid=" + base64_encode(data['data']['order_id']) + "&img=" + data['data']['qrcode_img'];
                            }
                        } else if (data["status"] == "pending" && data["error"] == "token failed") {
                            $scope.relogin()
                        } else {
    ...
    .controller('tksq', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        // $scope.count =  1;
        $scope.jjnum = function(arr){
            var inputval = parseInt($(".tk-center .t-c-1of5").find("input").val());
            if(arr=='jia'){
                if(inputval<$scope.shopnum){
                  inputval++;
                }else{
                  inputval = $scope.shopnum; 
                }
                ...
                if (isNaN(inputval) || inputval < 1) {
                    inputval = 1;
                } else if(inputval>$scope.shopnum){
                    inputval = $scope.shopnum;
                }else{
                    inputval = inputval;
                }
            }
            $(".tk-center .t-c-1of5").find("input").val(inputval)
        },
        $scope.tksq = function () {
            var images = '';
            var zrr = [];
            ...
            $http.post($scope.url + "/orderInterfaces.api?refundOrderNoFile", $.param({
                ...
                refund_count: $(".tk-center .t-c-1of5").find("input").val(),
    ...
    .controller('tixian', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $scope.txbtn = function (arr, brr, crr) {//提现金额 支付密码 验证码
            ...
            $http.post($scope.url + "/memberInterfaces.api?applyCash", $.param({
                ...
                if (data["status"] == "ok") {
                    $scope.alerttxt('申请成功')
                    window.location.href = "core.html#/wdqb"
                } else if (data["status"] == "pending" && data["error"] == "token failed") {
                    $scope.relogin()
                } else {
                    $scope.alerttxt(data['error'])
    ...
    .controller('shdz', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
     ...
     //发票管理
    .controller('fp', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore,$window) {
        $scope.scrolltop(0)
        $scope.h3nav(1, 1);
        //初始化编辑操作的addid
        $scope.editaddid = 0;
        ...
        $scope.tckfphide = function(){
           $(".tck.addhide").hide();
           $(".tck.edithide").hide();
        //    $(".fptck-box").find('.fpzl span').eq(0).addClass('act');
        //    $(".fptck-box").find('.fpzl span').eq(1).removeClass('act');
        //   $window.location.reload();
        ...
        $scope.keepfpxx = function(arr){
             ...
             data.member_token = $cookieStore.get("member_token");
             if(arr=="0"){
                var  len= $(".fptck-box").find('.fpzl span').length;
                for(var i=0;i<len;i++){
                    //循环发票
                    if($(".fptck-box").find('.fpzl span').eq(i).hasClass('act')){
                        ...
                        if(val=='zz'){
                            ...
                            //发票内容
                            var content_len1 = $(".address-box .fptck-box").find('.center').find('.zz .zz-fpcenter span').length;
                            var contentdata1 = [];
                            for (var j = 0; j < content_len1; j++) {
                                if ($(".address-box .fptck-box").find('.center').find('.zz .zz-fpcenter span').eq(j).hasClass('act')) {
                                contentdata1.push($(".address-box .fptck-box").find('.center').find('.zz .zz-fpcenter span').eq(j).text())
                                }
                            }
                            data.invoice_type ='paper';
                            data.invoice_content = contentdata1.join(',');
                ...
                for(var i=0;i<len;i++){
                    ...
                    if($(".address-box-edit .fptck-box").find('.fpzl span').eq(i).hasClass('act')){
                        ...
                        if(val=='zz'){
                            ...
                            //发票内容
                            var content_len = $(".tck .address-box-edit .fptck-box").find('.center').find('.zz .zz-fpcenter span').length;
                            var contentdata = [];
                            for (var j = 0; j < content_len; j++) {
                                if ($(".tck .address-box-edit .fptck-box").find('.center').find('.zz .zz-fpcenter span').eq(j).hasClass('act')) {
                                contentdata.push($(".tck .address-box-edit .fptck-box").find('.center').find('.zz .zz-fpcenter span').eq(j).text())
                                }
                            }
                            data.invoice_type ='paper';
        ...
        $scope.fplist = function(){
            $http.post($scope.url + "/memberInterfaces.api?getMemberInvoiceList", $.param({
                ...
                if (data["status"] == "ok") {
                    ...
                    $scope.fplength = data['data'].length;
                    console.log($scope.fplists)
                } else if (data["status"] == "pending" && data["error"] == "token failed") {
                    $scope.relogin()
                } else {
                    $scope.alerttxt(data['error'])
    ...
    .controller('qbcz', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        //支付  付钱-->ping++
        // $scope.pingpay=function(){//订单号  支付方式
        //   if(brr=="alipay_pc_direct"){
                ...
                $http.post($scope.url + "/orderInterfaces.api?insertRechargeOrder", $.param({
                    ...
                    if (data["status"] == "ok") {
                        if (brr == "alipay_pc_direct") {
                            ...
                            location.href = "gwc.html#/wxpay?paynum=" + base64_encode(arr) + "&orderid=" + base64_encode(data['data']['order_id']) + "&img=" + data['data']['qrcode_img']
                        }
                    } else if (data["status"] == "pending" && data["error"] == "token failed") {
                        $scope.relogin()
                    } else {
    ...
    .controller('wdrz', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $scope.wdrzbtn = function () {
            ...
            if (x == false && $scope.qiyeIndex == true) {
                $scope.alerttxt('请选择同意我已阅读相关细则');
                return false;
            }
            ...
            $http.post($scope.url + "/merchantsInterfaces.api?applyMerchants", $.param({
                ...
                if (data["status"] == "ok") {
                    $scope.alerttxt('感谢您的申请！我们将在3个工作日内对您的信息进行确认与回复。');
                    $scope.cwdrz();
                } else if (data["status"] == "pending" && data["error"] == "token failed") {
                    $scope.relogin()
                } else {
    ...
    .controller('xyrz', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore) {
        ...
        $scope.wdrzbtn = function () {
            var x = $("input[type='checkbox']").is(':checked')
            if (x == false) {
                $scope.alerttxt('请选择我已阅读并同意信用支付协议');
                return false;
            }
            if ($(".xm").children(".ul_list")) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    if (x.eq(i).find('.xmname').val() == '') {
                        $scope.alerttxt('请输入第' + (i * 1 + 1 * 1) + '个基金的基金名称')
                        return false;
                    }
                    xm.item_name = x.eq(i).find('.xmname').val();
                    if (x.eq(i).find('.fzname').val() == '') {
                        $scope.alerttxt('请输入第' + (i * 1 + 1 * 1) + '个基金的负责人')
                        return false;
                    }
                    xm.item_responsible_name = x.eq(i).find('.fzname').val();
                    if (x.eq(i).find('.dwname').val() == '') {
                        $scope.alerttxt('请输入第' + (i * 1 + 1 * 1) + '个基金的单位名称')
                        return false;
                    }
                    xm.company_name = x.eq(i).find('.dwname').val();
                    if (x.eq(i).find('.xmcode').val() == '') {
                        $scope.alerttxt('请输入第' + (i * 1 + 1 * 1) + '个基金的基金批准号')
                        return false;
                    }
                    xm.item_code = x.eq(i).find('.xmcode').val();
                    // 新加项目类型
                    // if(x.eq(i).find('.xmsort').val()==''){
                    //     $scope.alerttxt('请输入第'+(i*1+1*1)+'个项目的项目类型')
                    //     return false;
                    // }
                    xm.item_sort = x.eq(i).find('.xmsort').val();
                    ...
                    if (x.eq(i).find('.xmmoney').val() == '') {
                        $scope.alerttxt('请输入第' + (i * 1 + 1 * 1) + '个基金的基金金额')
                        return false;
                    }
    ...
    .controller('bzzx', function ($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore, $sce) {
        ...
        $scope.bzzxfinds = function (arr) {
            $http.post($scope.url + "/othersInterfaces.api?getHtmlDesc", $.param({
                ...
                $scope.bzzxtxt = $sce.trustAsHtml(data);//==''?'波尔生物很高兴能帮到你':data
    ...
    .directive("addxm", [function () {  //
        return {
            link: function (scope, element, attributes) {
                element.click(function () {
                    var tpl = '<ul class="ul_list">' +
                        '<li class="w-l-2of"><label>基金名称：</label><input class="xmname" type="text" /><span class="del">删除</span></li>' +
                        ...
                        '<li class="w-l-2of"><label>基金批准号：</label><input class="xmcode" type="text" /></li>' +
                        '  <li class="w-l-2of"><label>基金类型：</label><input class="xmsort" type="text" placeholder="可选填"/></li>' +
                        '<li class="w-l-2of"><label>批准时间（起点年月）：</label><input class="xmtime1" onClick="WdatePicker()" placeholder="可选填"/></li>' +
                        '<li class="w-l-2of"><label>批准时间（终止年月）：</label><input class="xmtime2" onClick="WdatePicker()" placeholder="可选填" /></li>' +
                        '<li class="w-l-2of"><label>基金金额（万元）：</label><input class="xmmoney" type="number" /></li>' +
                        '<li class="w-l-2of"><label>授信额度（万元）：</label><input disabled class="xmmoney2" type="text" /><span class="red mar-l20">授信额度&nbsp;=&nbsp;基金金额&nbsp;*&nbsp;10%</span></li>' +
                        '<li class="w-l-2of" style="margin-bottom:0px;"><label>备注：</label><textarea class="xmtext"></textarea></li>' +
                        '</ul>'
